...
class CorrectName:
    ...
    def validate(self, value):
        ...
        if bool(re.search(pattern, value)) or not value.istitle():
            raise AttributeError(f'Должно содержать только буквы и начинаться с большой буквы.')
...
class CorrectNum:
    ...
    def validate(self, value):
        if not self.__min_value <= value <= self.__max_value:
            raise ValueError(f'Значение должна быть {self.__min_value}-{self.__max_value}')
